...
class TouchstonePlotter(HasTraits):
    ...
    form = Enum('Mag (dB)', ['Mag (dB)', 'Phase (deg)', 'Smith', 'Mag (lin)',
                             'Phase (lin)', 'Real', 'Imag'])
    ...
    def _plot_button_fired(self):
        ...
        self.active_ntwk.__getattribute__('plot_{}_{}'.format(self.param, form))(m=m, n=n)
        plb.draw()
        plb.show()

    view = View(Item('cur_dir'), HSplit(Item('active_ntwk_name',
                                             show_label=False),
