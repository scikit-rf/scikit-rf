...
class MediaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        '''
        '''
        self.files_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                      'qucs_prj')
        self.dummy_media = rf.media.Media(frequency=rf.Frequency(1, 100, 21, 'ghz'),
    ...
    def test_impedance_mismatch(self):
        '''
        '''
        fname = os.path.join(self.files_dir, 'impedanceMismatch,50to25.s2p')
    ...
    def test_resistor(self):
        '''
        '''
        fname = os.path.join(self.files_dir, 'resistor,1ohm.s2p')
        qucs_ntwk = rf.Network(fname)
        self.dummy_media.frequency = qucs_ntwk.frequency
    ...
    def test_capacitor(self):
        '''
        '''
        fname = os.path.join(self.files_dir, 'capacitor,p01pF.s2p')
        qucs_ntwk = rf.Network(fname)
        self.dummy_media.frequency = qucs_ntwk.frequency
    ...
    def test_inductor(self):
        '''
        '''
        fname = os.path.join(self.files_dir, 'inductor,p1nH.s2p')
        qucs_ntwk = rf.Network(fname)
        self.dummy_media.frequency = qucs_ntwk.frequency
