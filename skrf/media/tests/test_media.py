...
class DefinedGammaZ0TestCase(unittest.TestCase):
    ...
    def test_resistor_sdef(self):
        """
        Compare the component values against s-parameters generated by QUCS.
        The different s_def QUCS component is generated by renormalize() method.
        """
        # Config the qucs_ntwk and media
        name = 'resistor,1ohm'
        qucs_ntwk = Network(os.path.join(self.files_dir, name + '.s2p'))
        ...
        for s_def in s_defs:
            qucs_ntwk_copy = qucs_ntwk.copy()
            qucs_ntwk_copy.renormalize(z0, s_def=s_def)
            skrf_ntwk = self.dummy_media.resistor(1, name=name, z0=z0, s_def=s_def)
            skrf_ntwk_y = resistor_y_def(self.dummy_media, 1, z0=z0, s_def=s_def)
    ...
    def test_capacitor_sdef(self):
        ...
        for s_def in s_defs:
            qucs_ntwk_copy = qucs_ntwk.copy()
            qucs_ntwk_copy.renormalize(z0, s_def=s_def)
            skrf_ntwk = self.dummy_media.capacitor(.01e-12, name=name, z0=z0, s_def=s_def)
            skrf_ntwk_y = capacitor_y_def(self.dummy_media, .01e-12, z0=z0, s_def=s_def)
            self.assertEqual(skrf_ntwk, qucs_ntwk_copy)
            self.assertEqual(skrf_ntwk.s_def, qucs_ntwk_copy.s_def)
            assert_array_almost_equal(skrf_ntwk.s, skrf_ntwk_y.s)
            self.assertEqual(skrf_ntwk.s_def, skrf_ntwk_y.s_def)
    ...
    def test_inductor_sdef(self):
        ...
        for s_def in s_defs:
            qucs_ntwk_copy = qucs_ntwk.copy()
            qucs_ntwk_copy.renormalize(z0, s_def=s_def)
            skrf_ntwk = self.dummy_media.inductor(.1e-9, name=name, z0=z0, s_def=s_def)
            skrf_ntwk_y = inductor_y_def(self.dummy_media, .1e-9, z0=z0, s_def=s_def)
            self.assertEqual(skrf_ntwk, qucs_ntwk_copy)
            self.assertEqual(skrf_ntwk.s_def, qucs_ntwk_copy.s_def)
