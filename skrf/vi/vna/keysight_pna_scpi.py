...
class SCPI(object):
    ...
    def set_delete_meas(self, cnum="", mname=""):
        scpi_command = scpi_preprocess(":CALC{:}:PAR:DEL '{:}'", cnum, mname)
        self.resource.write('{:}'.format(scpi_command))

    def set_display_format(self, cnum="", fmt=""):
    ...
    def set_averaging_mode(self, cnum="", avg_mode=""):
        scpi_command = scpi_preprocess(":SENS{:}:AVER:MODE {:}", cnum, avg_mode)
        self.resource.write('{:}'.format(scpi_command))

    def set_averaging_count(self, cnum="", avg_count=""):
    ...
    def query_display_format(self, cnum=""):
