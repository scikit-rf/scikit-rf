...
class PNA(Driver):
    ...
    #    Go to local.
    #    '''
    #    self._vpp43.gpib_control_ren(self.vi,
    ...
    def get_oneport(self, port=1, *args, **kwargs):
        ...
        self.continuous = False
        self.sweep()
        ntwk = Network(s=self.get_data_snp(port),
                       frequency=self.get_frequency(),
                       *args, **kwargs)
        self.continuous = was_cont
        return ntwk
    ...
    def get_twoport(self, ports=[1, 2], *args, **kwargs):
        ...
        self.continuous = False
        self.sweep()
        ntwk = Network(s=self.get_data_snp(ports),
                       frequency=self.get_frequency(),
                       *args, **kwargs)
        self.continuous = was_cont
        return ntwk
    ...
    def display_trace(self, name='', window_n=None, trace_n=None, form='MLOG'):
        ...
        if trace_n is None:
            trace_n = self.ntraces+1
        self.write('disp:wind%s:trac%s:feed \"%s\"'%(str(window_n),
                                                     str(trace_n),
                                                     name))
        self.select_meas(name)
    ...
    def get_cset_list(self, form='name'):
        ...
        '''
        a = self.ask('SENSE%i:CORRection:CSET:CATalog? %s'%(self.channel, form))
        return a[1:-1].split(',')
    ...
    def get_active_cset(self, channel=None, form='name'):
        ...
        out = self.ask('SENS%i:CORR:CSET:ACT? %s'%(channel, form))[1:-1]
    ...
    def set_active_cset(self, cset, channel=None, apply_stim_values=True):
        ...
        self.write('SENS%i:CORR:CSET:ACT \'%s\',%i'%(channel,
                                                     cset,
                                                     int(apply_stim_values)))
...
class VectorStar(PNA):
    ...
    def get_twoport(self, *args, **kwargs):
        ...
        # i dont know what the first 19 values are
        d = self.ask_for_values("TRS;WFS;OS2P")[19:]
        d = npy.array(d)
        d = d.reshape(-1, 9)
        s11 = d[:, 1] +1j*d[:, 2]
        s21 = d[:, 3] +1j*d[:, 4]
...
class HP8510C(Driver):
    ...
    def frequency(self, unit='ghz'):
        ...
        freq = Frequency(float(self.ask('star;outpacti;')),
                         ...
                         int(float(self.ask('poin;outpacti;'))),
                         'hz')
        freq.unit = unit
        return freq
    ...
    def two_port(self):
        ...
        s21 = self.s21.s[:, 0, 0]
        ntwk = Network()
        ntwk.s = npy.array([[s11, s21],
                            [s12, s22]]).transpose().reshape(-1, 2, 2)
        ntwk.frequency = self.frequency
        return ntwk
