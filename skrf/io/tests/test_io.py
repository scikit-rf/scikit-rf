...
class IOTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.embeding_network = rf.Network(os.path.join(self.test_dir,
                                                        'embedingNetwork.s2p'))
    ...
    def test_readwrite_calibration(self):
        ...
        cal = rf.Calibration(ideals=ideals,
                             measured=measured,
                             type='one port',
                             is_reciprocal=True)
        original = cal
    ...
    def test_readwrite_media(self):
        a_media = rf.media.Media(frequency=self.freq, 
    ...
    def test_readwrite_RectangularWaveguide(self):
        a_media = rf.media.RectangularWaveguide(frequency=self.freq, 
                                                a=100*rf.mil,
                                                z0=50)
        self.read_write(a_media)
    ...
    def test_readwrite_DistributedCircuit(self):
        ...
        a_media = rf.media.DistributedCircuit(frequency=self.freq,  
                                              R=1e5*one,
                                              G=1*one,
                                              I=1e-6*one,
                                              C=8e-12*one)
    ...
    def test_readwrite_Freespace(self):
        a_media = rf.media.Freespace(frequency=self.freq)
        self.read_write(a_media)
