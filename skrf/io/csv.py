...
class AgilentCSV(object):
    ...
    def read(self):
        ...
        fid.close()
        try:
            data = npy.genfromtxt(self.filename,
                                  delimiter=',',
                                  skip_header=begin_line + 2,
                                  skip_footer=footer,
                                 )
    ...
    def scalar_networks(self):
        ...
        for k in range(1, n_traces+1):
            ...
            if 'db' in cols[k].lower():
                s = mf.db_2_mag(s)
            ntwk_list.append(Network(frequency=freq, s=s, comments=comments,
        ...
        if (self.n_traces)/2 == 0:
            ...
            for k in range((self.n_traces)/2):
                ...
                if 'db' in names[k].lower() and 'deg' in names[k+1].lower():
                    ...
                    s = d[:, k*2+1]+1j*d[:, k*2+2]
                ntwk_list.append(Network(frequency=self.frequency,
                                         s=s,
                                         name=name,
                                         comments=comments,
    ...
    def dataframe(self):
        ...
        index = Index(self.frequency.f_scaled,
                      name='Frequency(%s)'%self.frequency.unit)
        return DataFrame({self.columns[k]:self.data[:, k]\
    ...
    if (d.shape[1]-1)/2 == 0:
        ...
        for k in range((d.shape[1]-1)/2):
            ...
            if 'db' in names[k].lower() and 'deg' in names[k+1].lower():
                ...
                s = d[:, k*2+1]+1j*d[:, k*2+2]
            ntwk_list.append(Network(f=f, s=s, name=name, comments=comments))
    return ntwk_list
...
def pna_csv_2_scalar_ntwks(filename, *args, **kwargs):
    ...
    for k in range(1, n_traces+1):
        ...
        if 'db' in cols[k].lower():
            s = mf.db_2_mag(s)
        ntwk_list.append(Network(frequency=freq,
                                 s=s,
                                 comments=comments,
                                 name=cols[k],
                                 *args, **kwargs)
