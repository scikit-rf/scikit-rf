...
class NetworkSet(object):
    ...
    def __init__(self, ntwk_set, name=None):
        ...
        for network_method_name in ['write_touchstone',
                                    'interpolate',
                                    'plot_s_smith']:
            self.__add_a_element_wise_method(network_method_name)
        for operator_name in ['__pow__', '__floordiv__', '__mul__',
                              '__div__', '__add__', '__sub__']:
    ...
    def __add_a_operator(self, operator_name):
        ...
        def operator_func(self, other):
            if isinstance(other, NetworkSet):
                ...
                return NetworkSet([self.ntwk_set[k].__getattribute__(operator_name)
                                   (other.ntwk_set[k]) for k in range(len(self))])

            elif isinstance(other, Network):
                return NetworkSet([ntwk.__getattribute__(operator_name)(other)
    ...
    def __str__(self):
        '''
        '''
        output = 'A NetworkSet of length %i'%len(self.ntwk_set)
        return output
    ...
    def element_wise_method(self, network_method_name, *args, **kwargs):
        ...
        '''
        output = [ntwk.__getattribute__(network_method_name)(*args, **kwargs)
                  for ntwk in self.ntwk_set]
    ...
    def plot_uncertainty_bounds_component(self, attribute, m=None, n=None,
                                          type='shade', n_deviations=3,
                                          alpha=.3, color_error=None,
                                          markevery_error=20, ax=None, ppf=None,
                                          kwargs_error={}, *args, **kwargs):
        '''
        ...
        for m in M:
            for n in N:
                ylabel_dict = {'s_mag':'Magnitude',
                               's_deg':'Phase (deg)',
                               's_deg_unwrap':'Phase (deg)',
                               's_deg_unwrapped':'Phase (deg)',
                               's_db':'Magnitude (dB)'}
                ax = plb.gca()
                ntwk_mean = self.__getattribute__('mean_'+attribute)
                ntwk_std = self.__getattribute__('std_'+attribute)
                ...
                if type == 'shade':
                    ...
                    ax.fill_between(ntwk_mean.frequency.f_scaled,
                                    lower_bound, upper_bound, alpha=alpha,
    ...
    def plot_minmax_bounds_component(self, attribute, m=0, n=0, type='shade',
                                     alpha=.3, color_error=None,
                                     markevery_error=20, ax=None, ppf=None,
                                     kwargs_error={}, *args, **kwargs):
        '''
        ...
        if type == 'shade':
            ...
            if color_error is None:
                color_error = ax.get_lines()[-1].get_color()
            ax.fill_between(ntwk_mean.frequency.f_scaled, lower_bound,
                            upper_bound, alpha=alpha, color=color_error,
                            **kwargs_error)
            #ax.plot(ntwk_mean.frequency.f_scaled, ntwk_mean.s[:, m, n], *args, **kwargs)
        elif type == 'bar':
    ...
    def plot_uncertainty_bounds_s(self, multiplier=200, *args, **kwargs):
        ...
        [self.mean_s[k].plot_s_smith(*args, ms=self.std_s[k].s_mag*multiplier,
                                     **default_kwargs)
    ...
    def ntwk_attr_2_df(self, attr='s_db', m=0, n=0, *args, **kwargs):
        ...
        index = Index(self[0].frequency.f_scaled,
                      name='Freq(%s)'%self[0].frequency.unit)
        df = DataFrame(dict([('%s'%(k.name),
                              Series(k.__getattribute__(attr)[:, m, n],
                                     index=index)) for k in self]),
                       index=index)
        return df
...
def func_on_networks(ntwk_list, func, attribute='s', name=None,
                     *args, **kwargs):
    '''
    ...
    '''
    data_matrix = npy.array([ntwk.__getattribute__(attribute)
