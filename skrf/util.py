...
class HomoList(collections.Sequence):
    ...
    def __getattr__(self, name):
        ...
        return self.__class__([k.__getattribute__(name) for k in self.store])
...
class HomoDict(collections.MutableMapping):
    ...
    def __getattr__(self, name):
        ...
        return self.__class__({k: getattr(self.store[k], name)
                               for k in self.store})
    ...
    def __call__(self, *args, **kwargs):
        ...
        return self.__class__({k: self.store[k](*args, **kwargs)
                               for k in self.store})
