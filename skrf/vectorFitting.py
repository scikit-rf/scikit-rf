...
class VectorFitting:
    ...
    def get_spurious(poles: np.ndarray, residues: np.ndarray, n_freqs: int = 101, gamma: float = 0.03) -> np.ndarray:
        ...
        h = residues[:, None, :] / (1j * omega_eval[:, None] - poles) + np.conj(residues[:, None, :]) / (
            1j * omega_eval[:, None] - np.conj(poles)
        )
        ...
        while iterations > 0:
            ...
            if delta_max < self.max_tol:
                if converged:
                    ...
                    logging.info(
                        f"Pole relocation process converged after {self.max_iterations - iterations + 1} " "iterations."
                    )
                    stop = True
                else:
            ...
            if iterations == 0:
                ...
                if max_cond > 1e10:
                    hint_illcond = (
                        "\nHint: the linear system was ill-conditioned (max. condition number was " f"{max_cond})."
                    )
                else:
                    hint_illcond = ""
                if max_deficiency < 0:
                ...
                if converged and stop is False:
                    warnings.warn(
                        ...
                        "The results might not have converged properly." + hint_illcond + hint_rank,
                        RuntimeWarning,
                        stacklevel=2,
                    )
                else:
                    warnings.warn(
                        ...
                        "The results did not converge properly." + hint_illcond + hint_rank,
                        RuntimeWarning,
                        stacklevel=2,
                    )
    ...
    def auto_fit(
        ...
        delta = self._get_delta(
            poles, residues, constant_coeff, proportional_coeff, freqs_norm, freq_responses, weights_responses
        )
    ...
    def _init_poles(freqs: list, n_poles_real: int, n_poles_cmplx: int, init_pole_spacing: str):
        ...
        if init_pole_spacing == "log":
            ...
            warnings.warn(
                "Invalid choice of initial pole spacing; proceeding with linear spacing.", UserWarning, stacklevel=2
            )
            pole_freqs_real = np.linspace(fmin, fmax, n_poles_real)
            pole_freqs_cmplx = np.linspace(fmin, fmax, n_poles_cmplx)
    ...
    def _fit_residues(poles, freqs, freq_responses, fit_constant, fit_proportional):
        ...
        x, residuals, rank, singular_vals = np.linalg.lstsq(
            np.vstack((A.real, A.imag)), np.hstack((freq_responses.real, freq_responses.imag)).T, rcond=None
        )
    ...
    def _get_delta(poles, residues, constant_coeff, proportional_coeff, freqs, freq_responses, weights_responses):
        ...
        for i, pole in enumerate(poles):
            if np.imag(pole) == 0.0:
                ...
                model += residues[:, i, None] / (s - pole) + np.conjugate(residues[:, i, None]) / (
                    s - np.conjugate(pole)
                )
    ...
    def _get_s_from_ABCDE(
        freqs: np.ndarray, A: np.ndarray, B: np.ndarray, C: np.ndarray, D: np.ndarray, E: np.ndarray
    ) -> np.ndarray:
        """
        Private method.
    ...
    def write_npz(self, path: str) -> None:
        ...
        if self.proportional_coeff is None:
            warnings.warn(
                "Nothing to export; Proportional coefficients have not been fitted.", RuntimeWarning, stacklevel=2
            )
            return
        if self.constant_coeff is None:
        ...
        np.savez_compressed(
            os.path.join(path, f"coefficients_{filename}"),
            poles=self.poles,
    ...
    def get_model_response(self, i: int, j: int, freqs: Any = None) -> np.ndarray:
        ...
        if self.proportional_coeff is None:
            warnings.warn(
                "Returning a zero-vector; Proportional coefficients have not been fitted.", RuntimeWarning, stacklevel=2
            )
            return np.zeros_like(freqs)
        if self.constant_coeff is None:
    ...
    @axes_kwarg
    def plot(
        self, component: str, i: int = -1, j: int = -1, freqs: Any = None, parameter: str = "s", *, ax: Axes = None
    ) -> Axes:
        """
        ...
        if component.lower() in components:
            ...
            if self.network is not None:
                ...
                if parameter.lower() == "s":
                    ...
                    raise ValueError(
                        "The network parameter type is not valid, must be `s`, `z`, or `y`, " f"got `{parameter}`."
                    )
    ...
    def plot_s_singular(self, freqs: Any = None, *, ax: Axes = None) -> Axes:
        ...
        for n in range(n_ports):
            ax.plot(freqs, sigma[:, n], label=rf"$\sigma_{n + 1}$")
        ax.set_xlabel("Frequency (Hz)")
        ...
        with open(file, "w") as f:
            ...
            for n in range(self.network.nports):
                ...
                for j in range(self.network.nports):
                    ...
                    f.write(
                        f"F{n + 1}{j + 1} 0 a{n + 1} V{n + 1}{j + 1}"
                        f"{formatter(1 / np.real(self.network.z0[0, n]))}\n"
                    )
                    f.write(
                        f"F{n + 1}{j + 1}_inv a{n + 1} 0 V{n + 1}{j + 1}_inv "
                        f"{formatter(1 / np.real(self.network.z0[0, n]))}\n"
                    )
                    ...
                    for i_pole in range(len(self.poles)):
                        ...
                        if np.imag(pole) == 0.0:
                            ...
                            if gm_add < 0:
                                ...
                                m = 1
                            f.write(
                                node + f" 0 rcl_vccs_admittance res={formatter(r)} cap={formatter(c)} "
