...
class Network(object):
    ...
    COMPONENT_FUNC_DICT = {'re': npy.real,
                           'im': npy.imag,
                           'mag': npy.abs,
                           'db': mf.complex_2_db,
                           ...
                           'deg_unwrap': lambda x: mf.radian_2_degree(\
                                            mf.unwrap_rad(npy.angle(x))),
                           'arcl_unwrap': lambda x: mf.unwrap_rad(\
                                            npy.angle(x))* npy.abs(x),
                           'vswr': lambda x: (1+abs(x))/(1-abs(x)),
                           'time': lambda x: fft.ifftshift(fft.ifft(x, axis=0),
                                                           axes=0),
                           'time_db': lambda x: mf.complex_2_db(\
                                            fft.ifftshift(fft.ifft(x, axis=0),
                                                          axes=0)),
                           'time_mag': lambda x: mf.complex_2_magnitude(\
                                            fft.ifftshift(fft.ifft(x, axis=0),
                                                          axes=0)),
                          }
    # provides y-axis labels to the plotting functions
    global Y_LABEL_DICT
    Y_LABEL_DICT = {'re': 'Real Part',
                    'im': 'Imag Part',
                    'mag': 'Magnitude',
                    'abs': 'Magnitude',
                    'db': 'Magnitude (dB)',
    ...
    ## CONSTRUCTOR
    def __init__(self, file=None, name=None, comments=None, f_unit=None,
                 **kwargs):
        '''
        Network constructor.
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, Network):
            ...
            result.s = self.s*npy.array(other).reshape(-1,
                                                       self.nports,
                                                       self.nports)
        return result
    ...
    def __rsub__(self, other):
        ...
        if isinstance(other, Network):
            ...
            # other may be an array or a number
            result.s = npy.array(other).reshape(-1,
                                                self.nports,
                                                self.nports)-self.s
        return result
    ...
    def __generate_plot_functions(self):
        ...
        '''
        for prop_name in PRIMARY_PROPERTIES:
            def plot_prop_polar(self, m=None, n=None, ax=None, show_legend=True,
                                prop_name=prop_name, *args, **kwargs):
                ...
                for m in M:
                    for n in N:
                        ...
                        plot_complex_polar(z=getattr(self, prop_name)[:, m, n],
    ...
    '''%(prop_name, prop_name)
            setattr(self.__class__, 'plot_%s_polar'%(prop_name), \
                plot_prop_polar)
            def plot_prop_rect(self, m=None, n=None, ax=None, show_legend=True,
                               prop_name=prop_name, *args, **kwargs):
                # create index lists, if not provided by user
                if m is None:
                    M = range(self.number_of_ports)
                ...
                for m in M:
                    for n in N:
                        ...
                        if gen_label:
                            ...
                            kwargs['label'] = label_string
                        # plot the desired attribute vs frequency
                        plot_complex_rectangular(z=getattr(self, prop_name)[:, m, n],
                                                 show_legend=show_legend,
                                                 ax=ax,
                                                 *args, **kwargs)
                #if was_interactive:
    ...
    '''%(prop_name, prop_name)
            ...
            for func_name in COMPONENT_FUNC_DICT:
                ...
                def plot_func(self, m=None, n=None, ax=None, show_legend=True,
                              attribute=attribute, y_label=y_label,
                              *args, **kwargs):
                    # create index lists, if not provided by user
                    if m is None:
                        M = range(self.number_of_ports)
        ...
        >>> myntwk.plot_%s(m=1, n=0, color='r')
        '''%(attribute, attribute)
                setattr(self.__class__, 'plot_%s'%(attribute), plot_func)
    ...
    def interpolate(self, new_frequency, **kwargs):
        ...
        **kwargs : keyword arguments
        ...
        interpolation_s_re = interp1d(self.frequency.f,
                                      self.s_re, axis=0,
                                      **kwargs)
        interpolation_s_im = interp1d(self.frequency.f,
                                      self.s_im, axis=0,
                                      **kwargs)
        interpolation_z0_re = interp1d(self.frequency.f,
                                       self.z0.real, axis=0,
                                       **kwargs)
        interpolation_z0_im = interp1d(self.frequency.f,
                                       self.z0.imag, axis=0,
                                       **kwargs)
    ...
    def time_gate(self, t_start, t_stop, window=('kaiser', 6)):
        ...
        #normalize output
        gated.s = gated.s*npy.sum(self.s_mag, axis=0)/npy.sum(gated.s_mag,
                                                              axis=0)
        return gated

    # plotting
    def plot_s_smith(self, m=None, n=None, r=1, ax=None, show_legend=True,
                     chart_type='z', draw_labels=False, label_axes=False,
                     *args, **kwargs):
        '''
    ...
    def multiply_noise(self, mag_dev, phase_dev, **kwargs):
        ...
        '''
        phase_rv = stats.norm(loc=0, scale=phase_dev).rvs(size=self.s.shape)
        mag_rv = stats.norm(loc=1, scale=mag_dev).rvs(size=self.s.shape)
...
def connect(ntwkA, k, ntwkB, l, num=1):
    ...
    if assert_z0_at_ports_equal(ntwkA, k, ntwkB, l) == False:
        ntwkC.s = connect_s(ntwkA.s,
                            k,
                            impedance_mismatch(ntwkA.z0[:, k],
                                               ntwkB.z0[:, l]),
                            0)
    ...
    ntwkC.z0 = npy.hstack((npy.delete(ntwkA.z0,
                                      range(k, k+1), 1),
                           npy.delete(ntwkB.z0,
                                      range(l, l+1),
                                      1)))
...
def connect_fast(ntwkA, k, ntwkB, l):
    ...
    # combine z0 arrays and remove ports which were `connected`
    ntwkC.z0 = npy.hstack((npy.delete(ntwkA.z0, range(k, k+num), 1),
                           npy.delete(ntwkB.z0, range(l, l+num), 1)))
    return ntwkC
...
def stitch(ntwkA, ntwkB, **kwargs):
    ...
    '''
    A, B = ntwkA, ntwkB
    C = Network(frequency=Frequency.from_f(npy.r_[A.f[:], B.f[:]],
                                           unit='hz'),
                s=npy.r_[A.s, B.s],
                z0=npy.r_[A.z0, B.z0],
                name=A.name,
...
def one_port_2_two_port(ntwk):
    ...
    result.s[:, 0, 1] = npy.sqrt(1- npy.abs(s11)**2)*\
            npy.exp(1j*(npy.angle(s11)+npy.pi/2.*\
            (npy.angle(s11) < 0) -npy.pi/2*(npy.angle(s11) > 0)))
...
def n_oneports_2_nport(ntwk_list, *args, **kwargs):
    ...
    s_out = npy.concatenate([npy.concatenate([ntwk_list[(k+(l*nports))].s
                                              for k in range(nports)],
                                             2)
                             for l in range(nports)],
                            1)
    z0 = npy.concatenate([ntwk_list[k].z0
                          for k in range(0, nports**2, nports+1)],
                         1)
...
def s2t(s):
    ...
    '''
    #TODO: check rank(s) == 2
    t = npy.array([[-1*(s[:, 0, 0]*s[:, 1, 1]-s[:, 1, 0]*s[:, 0, 1])/s[:, 1, 0],
                    -s[:, 1, 1]/s[:, 1, 0]],
                   [s[:, 0, 0]/s[:, 1, 0],
                    1/s[:, 1, 0]]]).transpose()
    return t
...
def t2s(t):
    ...
    '''
    #TODO: check rank(s) == 2
    s = npy.array([[t[:, 0, 1]/t[:, 1, 1],
                    1/t[:, 1, 1]],
                   [(t[:, 0, 0]*t[:, 1, 1]- t[:, 1, 0]*t[:, 0, 1])/t[:, 1, 1],
                    -1*t[:, 1, 0]/t[:, 1, 1]]]).transpose()
    return s
...
def renormalize_s_pw(s, z_old, z_new):
    ...
    for fidx in xrange(s.shape[0]):
        ...
        B_ii = B[fidx]
        # Eq. 11, Eq. 12
        Q_ii = npy.sqrt(npy.absolute(B_ii.real/A_ii.real))\
                    *(A_ii+A_ii.conj())/(B_ii.conj()+A_ii) # Eq(11)
