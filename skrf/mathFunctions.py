...
def complex_components(z):
    ...
    return (npy.real(z), npy.imag(z),
            npy.angle(z, deg=True),
            complex_2_quadrature(z)[0],
            complex_2_quadrature(z)[1])
...
def sqrt_known_sign(z_squared, z_approx):
    ...
    return npy.where(npy.sign(npy.angle(z)) == npy.sign(npy.angle(z_approx)),
                     z,
                     z.conj())
...
def find_correct_sign(z1, z2, z_approx):
    ...
    '''
    return npy.where(npy.sign(npy.angle(z1)) == npy.sign(npy.angle(z_approx)),
                     z1,
                     z2)
