...
class Calibration(object):
    ...
    def __init__(self, measured, ideals, sloppy_input=False,
        ...
        if not self_calibration and len(self.measured) != len(self.ideals):
            raise IndexError('The length of measured and ideals lists are different. Number of ideals must equal the '
                             'number of measured. If you are using `sloppy_input` ensure the names are uniquely '
                             'alignable.')
        ...
        for k in list(range(len(self.ideals))):
            if self.ideals[k].frequency != self.measured[0]:
                print("Warning: Frequency information doesn't match on ideals[{}], attempting to interpolate the "
                      "ideal[{}] Network ..".format(k, k))
                try:
    ...
    * If more than 1 transmissive standard is given the `load match`,
    ...
    def __init__(self, measured, ideals, n_thrus=None, trans_thres=-40,
        ...
        if n_thrus is None:
            ...
            if n_thrus == 0:
                raise ValueError(
                    'couldnt find a transimssive standard. check your data, or explicitly use `n_thrus` argument')
        ...
        for k in range(-n_thrus, len(ideals)):
            if ideals[k] is None:
                if (n_thrus is None) or (hasattr(ideals, 'keys')) or \
                    ...
                    raise ValueError(
                        'Cant use sloppy_input and have the ideal thru be None. measured and ideals must be lists, or dont use None for the thru ideal.')
...
class NISTMultilineTRL(EightTerm):
    ...
    def run(self):
        ...
        for m in range(fpoints):
            ...
            gamma[m] = (dl_vec.conj().transpose().dot(V_inv).dot(g_dl)) / (
            dl_vec.conj().transpose().dot(V_inv).dot(dl_vec))
            ...
            if self.k_method == 'marks':
                ...
                denom = 1 - CoA1 * S_thru[0, 0] - CoA2 * S_thru[1, 1] + CoA1 * CoA2 * \
                                                                        (S_thru[0, 0] * S_thru[1, 1] - S_thru[0, 1] *
                                                                         S_thru[1, 0])
                R1 = S_thru[0, 1] / denom
                R2 = S_thru[1, 0] / denom
            else:
    ...
    def run(self):
        ...
        for f in list(range(fLength)):
            ...
            for k in list(range(numStds)):
                m, i = mList[k][f, :, :], iList[k][f, :, :]  # 2x2 s-matrices
                Q[k * 4:k * 4 + 4, :] = npy.array([ \
                    [i[0, 0], i[1, 0], 0, 0, 1, 0, 0, 0, -m[0, 0] * i[0, 0], -m[0, 0] * i[1, 0], -m[0, 1] * i[0, 0],
                     -m[0, 1] * i[1, 0], -m[0, 0], 0, -m[0, 1]], \
                    [i[0, 1], i[1, 1], 0, 0, 0, 1, 0, 0, -m[0, 0] * i[0, 1], -m[0, 0] * i[1, 1], -m[0, 1] * i[0, 1],
                     -m[0, 1] * i[1, 1], 0, -m[0, 0], 0], \
                    [0, 0, i[0, 0], i[1, 0], 0, 0, 1, 0, -m[1, 0] * i[0, 0], -m[1, 0] * i[1, 0], -m[1, 1] * i[0, 0],
                     -m[1, 1] * i[1, 0], -m[1, 0], 0, -m[1, 1]], \
                    [0, 0, i[0, 1], i[1, 1], 0, 0, 0, 1, -m[1, 0] * i[0, 1], -m[1, 0] * i[1, 1], -m[1, 1] * i[0, 1],
                     -m[1, 1] * i[1, 1], 0, -m[1, 0], 0], \
                    ])
                # pdb.set_trace()
                M[k * 4:k * 4 + 4, :] = npy.array([ \
                    [0], \
