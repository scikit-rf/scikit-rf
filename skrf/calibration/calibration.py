...
class Calibration(object):
    ...
    def from_coefs(cls, frequency, coefs, **kwargs):
        ...
        # * `calibration.frequency` property evaluates correctly
        # * TRL.__init__() will not throw an error
        n = Network(frequency=frequency, s=rand_c(frequency.npoints, 2, 2))
        measured = [n, n, n]
...
class OnePort(Calibration):
    ...
    def run(self):
        ...
        for f in range(fLength):
            ...
            # m-vector at f
            m = npy.array([mList[k][f] for k in range(numStds)]).reshape(-1, 1)
            # i-vector at f
            i = npy.array([iList[k][f] for k in range(numStds)]).reshape(-1, 1)
...
class SDDLWeikle(OnePort):
    ...
    def run(self):
        ...
        Bp = Bp_re + Bp_im*1j
        B = Bp + w_s    #(10) C = Bp * (1./w_1p - alpha/w_1p.conj()) + alpha * Bp/Bp.conj() #(20)
        A = B - w_s + w_s*C #(6)
...
class PHN(OnePort):
    ...
    def run(self):
        # ideals (in impedance)
        a = s2z(self.ideals[0].s, 1).flatten() # half known i
...
class UnknownThru(EightTerm):
    ...
    def __init__(self, measured, ideals, *args, **kwargs):
        ...
        switch_terms : tuple of :class:`~skrf.network.Network` objects
            the pair of switch terms in the order (forward, reverse)
        '''
        EightTerm.__init__(self, measured=measured, ideals=ideals, *args, **kwargs)
    ...
    def run(self):
        ...
        if self.switch_terms is not None:
            ...
            warn('No switch terms provided')
            coefs.update({
                'forward switch term': npy.zeros(len(self.frequency),
                                                 dtype=complex),
                'reverse switch term': npy.zeros(len(self.frequency),
                                                 dtype=complex),
                })
        coefs.update({'k':k_})
        self.coefs = coefs
    ...
    if len(coefs) == 3:
        ...
        for k in coefs:
            ...
            if eterm  in ['Directivity', 'SourceMatch', 'ReflectionTracking']:
                coef_key = eterm+'(%i,%i)'%(port_map_refl[fr], port_map_refl[fr])
...
def error_dict_2_network(coefs, frequency, is_reciprocal=False, **kwargs):
    ...
    if len(coefs.keys()) == 3:
        ...
        forward = error_dict_2_network(p1, frequency=frequency,
                                       name='forward', 
                                       is_reciprocal=is_reciprocal, **kwargs)
