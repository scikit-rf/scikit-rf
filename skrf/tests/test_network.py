...
class NetworkTestCase(unittest.TestCase):
    ...
    To test for 2N-port de-embedding Meas, F and DUT are created such as:
    ::
        Meas == F ** DUT
            Meas              F            DUT
         +---------+     +---------+   +---------+
    ...
    def test_open_saved_touchstone(self):
        self.ntwk1.write_touchstone('ntwk1_saved', dir=self.test_dir)
        ntwk1_saved = rf.Network(os.path.join(self.test_dir, 'ntwk1_saved.s2p'))
        self.assertEqual(self.ntwk1, ntwk1_saved)
        os.remove(os.path.join(self.test_dir, 'ntwk1_saved.s2p'))

    def test_pickling(self):
        original_ntwk = self.ntwk1
        with open(os.path.join(self.test_dir, 'pickled_ntwk.ntwk'), 'wb') as fid:
            pickle.dump(original_ntwk, fid)
        with open(os.path.join(self.test_dir, 'pickled_ntwk.ntwk'), 'rb') as fid:
    ...
    def test_stitch(self):
        tmp = self.ntwk1.copy()
        tmp.f = tmp.f + tmp.f[0]
        rf.stitch(self.ntwk1, tmp)

    def test_cascade(self):
        self.assertEqual(self.ntwk1 ** self.ntwk2, self.ntwk3)
        self.assertEqual(self.F ** self.DUT, self.Meas)

    def test_deembed1(self):
        self.assertEqual(self.ntwk3 // self.ntwk1, self.ntwk2)
        self.assertEqual(self.ntwk3 // (self.ntwk1, ), self.ntwk2)
        self.assertEqual(self.ntwk3 // [self.ntwk1], self.ntwk2)
        self.assertEqual(self.Meas // self.F, self.DUT)

    def test_connect(self):
        self.assertEqual(rf.connect(self.ntwk1, 1, self.ntwk2, 0), self.ntwk3)
    ...
    def test_conversions(self):
        ...
        for test_z0 in (50, 10, 90+10j, 4-100j):
            for test_ntwk in (self.ntwk1, self.ntwk2, self.ntwk3):
                ...
                self.assertTrue((abs(rf.a2s(rf.s2a(ntwk.s, test_z0), test_z0)-ntwk.s) < self.tinyfloat).all())
                self.assertTrue((abs(rf.z2s(rf.s2z(ntwk.s, test_z0), test_z0)-ntwk.s) < self.tinyfloat).all())
                self.assertTrue((abs(rf.y2s(rf.s2y(ntwk.s, test_z0), test_z0)-ntwk.s) < self.tinyfloat).all())
                self.assertTrue((abs(rf.t2s(rf.s2t(ntwk.s)) - ntwk.s) < self.tinyfloat).all())
        self.assertTrue((abs(rf.t2s(rf.s2t(self.F.s)) - self.F.s) < self.tinyfloat).all())
    ...
    def test_yz(self):
        ...
        self.assertTrue((abs(rf.y2z(ntwk.y)-ntwk.z) < self.tinyfloat).all())
        self.assertTrue((abs(rf.y2s(ntwk.y, ntwk.z0)-ntwk.s) < self.tinyfloat).all())
        self.assertTrue((abs(rf.z2y(ntwk.z)-ntwk.y) < self.tinyfloat).all())
        self.assertTrue((abs(rf.z2s(ntwk.z, ntwk.z0)-ntwk.s) < self.tinyfloat).all())
    ...
    def test_div(self):
        ...
        # operating on numbers
        self.assertTrue(((a/2.0).s == npy.array([[[0.5+1j]], [[3/2.0+2j]]])).all())
        # operating on list
        self.assertTrue(((a/[1, 2]).s == npy.array([[[1+2j]], [[3/2.0+2j]]])).all())
    ...
    def test_slicer(self):
        ...
        b = a['2-5GHz']
        self.assertTrue((abs(b.frequency.f - [2E9, 4E9, 5E9]) < self.tinyfloat).all())
