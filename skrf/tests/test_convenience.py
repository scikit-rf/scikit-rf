...
class ConvenienceTestCase(unittest.TestCase):
    ...
    def test_is_hfss_touchstone(self):
        ...
        self.test_dir = os.path.dirname(os.path.abspath(__file__))+'/'
        # Some HFSS files
        self.assertTrue(rf.is_hfss_touchstone(os.path.join(self.test_dir,'hfss_oneport.s1p')))
        self.assertTrue(rf.is_hfss_touchstone(os.path.join(self.test_dir,'hfss_twoport.s2p')))
        self.assertTrue(rf.is_hfss_touchstone(os.path.join(self.test_dir,'hfss_threeport_DB.s3p')))
        # Touchstone file not from HFSS
        self.assertFalse(rf.is_hfss_touchstone(os.path.join(self.test_dir,'ntwk1.s2p')))
