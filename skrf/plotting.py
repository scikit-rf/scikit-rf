...
def smith(smithR=1, chart_type='z', draw_labels=False, border=False, ax=None):
    ...
    if not draw_labels:
        ...
        xLightList = plb.hstack([plb.logspace(2, -5, 8, base=.5),
                                 -1*plb.logspace(2, -5, 8, base=.5)])
    else:
        rLightList = plb.array([0.2, 0.5, 1.0, 2.0, 5.0])
        xLightList = plb.array([0.2, 0.5, 1.0, 2.0, 5.0, -0.2, -0.5, -1.0,
                                -2.0, -5.0])
    ...
    if draw_labels:
        ...
        if smithR is 1 and y_flip_sign is 1:
            ...
            deltax = plb.array([-0.17, -0.14, -0.06, 0., 0.02, -0.2, -0.2,
                                -0.08, 0., 0.03])
            deltay = plb.array([0., 0.03, 0.01, 0.02, 0., -0.02, -0.06, -0.09,
                                -0.08, -0.05])
...
def plot_rectangular(x, y, x_label=None, y_label=None, title=None,
                     show_legend=True, axis='tight', ax=None,
                     *args, **kwargs):
    '''
...
def plot_polar(theta, r, x_label=None, y_label=None, title=None,
               show_legend=True, axis_equal=False, ax=None,
               *args, **kwargs):
    '''
...
def plot_complex_rectangular(z, x_label='Real', y_label='Imag',
                             title='Complex Plane', show_legend=True,
                             axis='equal', ax=None,
                             *args, **kwargs):
    '''
...
def plot_complex_polar(z, x_label=None, y_label=None, title=None,
                       show_legend=True, axis_equal=False, ax=None,
                       *args, **kwargs):
    '''
...
def plot_smith(s, smith_r=1, chart_type='z', x_label='Real',
               ...
               axis='equal', ax=None, force_chart=False,
               *args, **kwargs):
    '''
    ...
    *args, **kwargs : passed to pylab.plot
    ...
    plot_complex_rectangular(s, x_label=x_label, y_label=y_label, title=title,
                             show_legend=show_legend, axis=axis, ax=ax,
                             *args, **kwargs)
...
def shade_bands(edges, y_range=None, cmap='prism', **kwargs):
    ...
    for k in range(len(edges)-1):
        plb.fill_between([edges[k], edges[k+1]], y_range[0], y_range[1],
                         color=cmap(1.0*k/len(edges)), **kwargs)
    plb.axis(axis)
...
def add_markers_to_lines(ax=None, marker_list=['o', 'D', 's', '+', 'x'],
                         markevery=10):
    '''
