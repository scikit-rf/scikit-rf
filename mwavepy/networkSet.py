...
class NetworkSet(object):
	...
	add_a_func_on_property. Ploting functions and other methods are 
	...
	def __init__(self, ntwk_set):
		...
		for network_property_name in \
			...
			self.__add_a_set_plot(network_property_name)
	...
	def __add_operator(self,operator):
		def operator(self, operator):
			new_set = [ntwk.__get_attribute__(operator) for ntwk in self.ntwk_list]
		return NetworkSet(new_set)
	...
	def __add_a_set_plot(self,network_property_name):
		'''
		dynamically adds a ploting method to the class. This is mostly 
		used internally to generate the class methods.
		the plotting method will call a Network plotting method for each
		element of the set. 
		
		takes:
			network_property_name: the name of the Network property to 
				plot, a string. the Network must provide a plotting 
				method named 'plot_network_property_name'.
				
		returns:
			na
		'''
		def plot_func(self, *args, **kwargs):
			[self[k].__getattribute__('plot_'+network_property_name)(*args,**kwargs) \
