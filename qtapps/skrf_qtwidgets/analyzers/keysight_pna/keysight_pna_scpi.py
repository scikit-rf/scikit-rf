...
class SCPI(object):
    def set_data(self, cnum="", format=""):
                scpi_command = scpi_preprocess(":CALC{:}:DATA {:}", cnum, format)
    ...
    def set_disp_format(self, cnum="", format=""):
                scpi_command = scpi_preprocess(":CALC{:}:FORM {:}", cnum, format)
                self.resource.write('{:}'.format(scpi_command))
    ...
    def query_data(self, cnum="", format=""):
                scpi_command = scpi_preprocess(":CALC{:}:DATA? {:}", cnum, format)
